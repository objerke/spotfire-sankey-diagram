...
Spotfire.initialize(async (mod) => {
    ...
    async function render(dataView, windowSize, xAxis, yAxis) {
        ...
        svgmod.querySelectorAll("g").forEach(function (g) {
            g.innerHTML = "";
        });
        ...
        function collectNodes(node) {
            ...
            let a = levels[node.level][node.key];
            if (!a) {
                a = levels[node.level][node.key] || {
                    nodes: [],

                    barName: node.formattedValue(),
                    totalValue: 0
                };

                levels[node.level][node.key] = a;
            }
        ...
        // Calculate total value for each bar.
        rows.forEach((row) => {
        ...
        Object.keys(levels).forEach(function (level, i) {
            ...
            Object.keys(segments)
                ...
                .forEach(function (barsegment, j) {
                    ...
                    barsegment.x = x;
                    barsegment.y = y;
                    barsegment.height = height;

                    currentY += height + barsegmentgap;
                });
        });

        // Calculate all paths between rectangles
        rows.forEach((row) => {
            let widthOfPath = row.continuous("Y").value();

            const path = row.categorical("X").value();
            for (var i = 0; i < path.length - 1; i++) {
                let from = levels[i][path[i].key];
				let to = levels[i + 1][path[i + 1].key];
                ...
                let path = [
                    ...
                    "L",
                    barsegment2.x,
                    barsegmentrow2.y + rowvalue * heightscale,
                    "C",
                    barsegment2.x - bargap / 4,
                    barsegmentrow2.y + rowvalue * heightscale,
                    barsegment1.x + barwidth + bargap / 4,
                    barsegmentrow1.y + rowvalue * heightscale,
                    barsegment1.x + barwidth,
                    barsegmentrow1.y + rowvalue * heightscale,
                    "Z"
                ].join(" ");
            ...
            path.forEach((pathElement, i) => {
                levels[i][pathElement.key].totalValue += value;
            });
        });
        ...
        cataxislevels.forEach(function (level, i) {
            ...
            rows.forEach(function (row, j) {
                ...
                var barsegment = bar.barsegments.find((obj) => {
                    return obj.label === rowlabelpart;
                });
        ...
        Object.keys(levels).forEach(function (level, i) {
            ...
            Object.keys(segments)
                ...
                .forEach(function (barsegment, j) {
                    ...
                    let rows = barsegment.nodes.map((n) => n.rows()).flat();
        ...
        /**
         * Render rows. Now all segments have x, y and height
         */
        Object.keys(levels).forEach(function (level, i) {
            let segments = levels[level];
            let currentY = 0;
        ...
        rows.forEach(function (row, j) {
            return;
            var rowvalue = Number(row.continuous("Y").value());
            var value = row.categorical("X").value();
            var rowcolor = row.color().hexCode;

            // let y = barheightcursor;
            // let barheightcursor += barsegmentrow.rowvalue * heightscale;

            for (var i = 0; i < value.length; i++) {
                var barsegment1 = levels[i][value[i].key];
                // var barsegment1 = bar1.barsegments.find( obj => { return obj.label === value[i].formattedValue() });
                // var barsegmentrow1 = barsegment1.rows.find( obj => { return obj.rowid === j });

                if (i + 1 < value.length) {
                    var bar2 = bars[i + 1];
                    var barsegment2 = bar2.barsegments.find((obj) => {
                        return obj.label === value[i + 1].formattedValue();
                    });
                    var barsegmentrow2 = barsegment2.rows.find((obj) => {
                        return obj.rowid === j;
                    });
